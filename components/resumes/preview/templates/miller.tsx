...
export function MillerTemplate({ resume, scale = 1 }: TemplateProps) {
  ...
  const renderSection = (name: string): React.ReactNode => {
    const sectionWrapper = (key: string, label: string, children: React.ReactNode) => (
      <section key={key} style={{ marginBottom: '16px' }}>
        <h2 style={sectionHeadingStyle}>{label}</h2>
        <hr style={{ border: 'none', borderTop: '1px solid #ddd', marginBottom: '8px' }} />
        {children}
      </section>
    )
    ...
    switch (name.toLowerCase()) {
      case 'summary':
        return profile?.summary
          ? sectionWrapper('summary', 'Summary', (
      ...
      case 'repositories':
        return repositories.length > 0
          ? sectionWrapper('repositories', 'Repositories', (
              <>
                {repositories.map(rr => (
    ...
    >
      {contact && (
        <header style={{ textAlign: 'center', marginBottom: '20px' }}>
          ...
          <p style={{ fontSize: '9px', color: '#555', letterSpacing: '0.02em' }}>{contactLine}</p>
        </header>
      )}
