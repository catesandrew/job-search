...
export function generateLatexOxfordResume(resume: Resume): string {
  ...
  const cdot = ' $\\cdot$ '
  const oxPersonal = [profile?.email, formatPhone(profile?.phone), profile?.location]
  ...
  const contactBlock = [oxPersonal, oxSocial].filter(Boolean).join('\\\\[1pt]\n')
...
{\\small\\color{oxgray}${contactBlock}}\\par
${body}
\\end{document}
`
}
